$(document).ready(function() {
  // Initialize Firebase
  var config = {
    apiKey: process.env.CALENDAR_APP_47185_FIREBASEAPP_API_KEY,
  ...
  $("#movieInputZip").keyup(function(e) {
    if (e.keyCode == 13) {
      //triggers when a user presses enter after filling out a valid zip
      var apikey = process.env.CALENDAR_APP_MOVIES_API_KEY;
      ...
      $.ajax({
        ...
        response.forEach(function(movie) {
          ...
          while (nearest_time == false) {
            ...
            var movieTime = moment(
              movie.showtimes[i].dateTime.slice(-6),
              "THH:mm"
            ).format("h:mm A");
        ...
        $("#movieContent").on("click", "#button-view", function() {
          ...
          add_event(
            ...
            movieBtn[0].innerText + " (Movie)",
            moment(movieBtn[2].innerText, "hh:mm A").format("HH:mm"),
            moment(movieBtn[2].innerText, "hh:mm A")
              .add(2, "hours")
              .format("HH:mm"),
            moment(movieBtn[1].innerText, "YYYY-MM-DD"),
            sessionStorage.getItem("zip"),
            sessionStorage.getItem("movie_zip")
          );
          $("#movieContent").html("");
          $("#start-input").val("");
          $("#movieInputZip").val("");
  ...
  function send_sign_form() {
    ...
    if (form == "Sign Up") {
      var errors = [];
      var str_field_check = city_state_name_verification(
        $("#inputCity")
          .val()
          .trim(),
        $("#inputState")
      ...
      var location_check = location_verification(
        $("#inputZip")
          .val()
          .trim()
      );
      var confirm_check = confirm_password(
        $("#inputPassword").val(),
        $("#inputConfirmPassword").val()
      );
      ...
      if (errors.length == 0) {
        ...
        $("#error-body").html("");
        $("#error-body").append(
          $("<div>").text("There are error(s) with your submission:")
        );
  ...
  function send_event_form() {
    ...
    var location_setting_event = $("#event-location")
      .val()
      .trim();
    if (location_setting_event != "default-location") {
      zip_dest_check = location_verification(
        $("#event_loc_inputZip")
          .val()
          .trim()
      );
      str_field_dest_check = city_state_name_verification(
        $("#event_loc_inputCity")
          .val()
          .trim(),
        $("#event_loc_inputState")
          .val()
          .trim(),
        even_titl
      );
    ...
    var location_setting_origin = $("#start-location")
      .val()
      .trim();
    if (location_setting_origin != "default-location") {
      zip_origin_check = location_verification(
        $("#event_origin_inputZip")
          .val()
          .trim()
      );
      str_field_origin_check = city_state_name_verification(
        $("#event_origin_inputCity")
          .val()
          .trim(),
        $("#event_origin_inputState")
          .val()
          .trim(),
        even_titl
      );
    ...
    if (errors.length == 0) {
      ...
      if (
        ...
        zip_origin = $("#event_origin_inputZip")
          .val()
          .trim();
      }
  ...
  function load_calendar(date, uid) {
    database
      ...
      .then(function(snapshot) {
        database
          .ref(uid + "/events/" + date.format("MMM"))
          .once("value")
          .then(function(snapshot_event) {
            var events = [];
            ...
            $.ajax({
              ...
              $.ajax({
                ...
                //Sets name
                $("#name-calendar").text(
                  sessionStorage.getItem("name") + "'s Calendar"
                );
                ...
                for (var i = 1; i < 8; i++) {
                  ...
                  if (
                    ...
                    if (response.weather["0"].main == "Snow") {
                      weather = "Snow";
                      weather_html =
                        "<i class = 'fas fa-snowflake' style = 'font-size:36px;color:lightblue'></i>";
                    } else if (
                      ...
                      weather = "Rain";
                      weather_html =
                        "<i class = 'fas fa-cloud-rain' style = 'font-size:36px;color:lightblue'></i>";
                    } else if (response.weather["0"].main == "Clouds") {
                      weather = "Clouds";
                      weather_html =
                        "<i class = 'fas fa-cloud' style = 'font-size:36px;color:lightblue'></i>";
                    } else {
                      weather = "Clear";
                      weather_html =
                        "<i class = 'fas fa-sun' style = 'font-size:36px;font-weight:bolder;color:yellow'></i>";
                    }
                    ...
                    if (i == 1) {
                      $("#day-1-date").html(
                        "<span class = 'day-display'>" +
                          current_date_2.format("dddd") +
                          "</span><br>" +
                          weather_html +
                          "<br> <span class = 'date-display'>" +
                          current_date_2.format("D") +
                          "</span>"
                      );
                    ...
                    weather_data.forEach(function(value) {
                      if (
                        ...
                        if (value.weather["0"].main == "Snow") {
                          ...
                          value.weather["0"].main == "Rain" ||
                          value.weather["0"].main == "Mist"
                        ) {
                          weather_html =
                            "<i class = 'fas fa-cloud-rain' style = 'font-size:36px;color:lightblue'></i>";
                          ...
                          weather_html =
                            "<i class = 'fas fa-sun' style = 'font-size:36px;color:yellow'></i>";
                        }
                      } else if (
                        dt.format("YYYY-MM-DD") + " 00:00:00" == value.dt_txt &&
                        moment(value.dt_txt, "YYYY-MM-DD HH:mm:ss") >=
                          moment().add(4, "days")
                      ) {
                        if (value.weather["0"].main == "Snow") {
                          weather_html =
                    ...
                    weather = "Unknown";
                    weather_html =
                      "<i class = 'fas fa-sun' style = 'font-size:36px;color:transparent;'></i>";
                  }
                  ...
                  if (
                    sessionStorage.getItem("weather") != "Clear" ||
                    moment().format("HH:mm") >= "17:30" ||
                    moment().format("HH:mm") <= "06:30"
                  ) {
                ...
                $("#scroll-body").scrollTop(
                  parseFloat(moment().format("HH")) * 116
                );
                ...
                for (var i = 0; i < events.length; i++) {
                  ...
                  var start_datetime_lookup =
                    events[i].start_date +
                    " " +
                    moment(events[i].start_time, "HH:mm").format("HH") +
                    ":00";
                  var end_datetime =
                    events[i].start_date + " " + events[i].end_time;

                  var focused_cell = $(
                    "td[data-datetime = '" + start_datetime_lookup + "']"
                  );
                  focused_cell.html("");
                  if (
                    moment(start_datetime, "YYYY-MM-DD HH:mm").diff(
                      moment(start_datetime_lookup, "YYYY-MM-DD HH:mm"),
                      "minutes"
                    ) >= 30
                  ) {
                    if (
                      ...
                      focused_cell.css(
                        "background",
                        "linear-gradient(to bottom, #212529 0%,#212529 50%,#212529 50%,#007bff 50%,#007bff 100%)"
                      );
                    } else {
                      focused_cell.css(
                        "background",
                        "linear-gradient(to bottom, white 0%,white 50%,white 50%,#007bff 50%,#007bff 100%)"
                      );
                    }
                    var title_div = $("<div>");
                    ...
                    title_div.attr(
                      ...
                      "Event Name: <b>" +
                        ...
                        "</b><br>Date: " +
                        moment(events[i].start_date, "YYYY-MM-DD").format(
                          "MMM D, YYYY"
                        ) +
                        "<br>Start Time: " +
                        moment(events[i].start_time, "HH:mm").format("h:mm a") +
                        "<br>End Time: " +
                        moment(events[i].end_time, "HH:mm").format("h:mm a") +
                        "<br>Coming From: " +
                        events[i].start_loc +
                        "<br>Going to: " +
                        events[i].end_loc +
                    ...
                    moment(end_datetime, "YYYY-MM-DD HH:mm").diff(
                      moment(start_datetime, "YYYY-MM-DD HH:mm"),
                      "minutes"
                    ) <= 30
                  ) {
                    if (
                      ...
                      focused_cell.css(
                        "background",
                        "linear-gradient(to bottom, #007bff 0%,#007bff 50%,#007bff 50%,#212529 50%,#212529 100%)"
                      );
                    } else {
                      focused_cell.css(
                        "background",
                        "linear-gradient(to bottom, #007bff 0%,#007bff 50%,#007bff 50%,white 50%,white 100%)"
                      );
                    }
                    var title_div = $("<div>");
                    title_div.css("position", "absolute");
                  ...
                  while (populating_event_cell == true) {
                    var focused_time = moment(
                      start_datetime_lookup,
                      "YYYY-MM-DD HH:mm"
                    )
                      .add(j, "hours")
                      .format("YYYY-MM-DD HH:mm");
                    var focused_added_cell = $(
                      "td[data-datetime = '" + focused_time + "']"
                    );
                    if (
                      moment(end_datetime, "YYYY-MM-DD HH:mm").diff(
                        moment(focused_time, "YYYY-MM-DD HH:mm"),
                        "minutes"
                      ) <= 0
                    ) {
                      ...
                      title_div.attr(
                        ...
                        "Event Name: <b>" +
                          ...
                          "<br>Start Time: " +
                          moment(events[i].start_time, "HH:mm").format(
                            "h:mm a"
                          ) +
                          "<br>End Time: " +
                          moment(events[i].end_time, "HH:mm").format("h:mm a") +
                ...
                if (current_weather == "Clouds") {
                  $("#myVideo").attr(
                    "src",
                    "assets/images/storm_clouds_timelapse.mp4"
                  );
                  ...
                  $("#myVideo").attr(
                    "src",
                    "assets/images/storm_raindrops_on_window.mp4"
                  );
                } else if (
                  moment().format("HH:mm") >= "17:30" ||
                  moment().format("HH:mm") <= "06:30"
                  ...
                  $("#myVideo").attr(
                    "src",
                    "assets/images/Rainbow_Nebula_4K_Motion_Background.mp4"
                  );
                ...
                $(".date-jump-submit").on("click", function(event) {
                  ...
                  if (
                    moment($("#date-jump-text").val(), "YYYY-MM-DD").isValid()
                  ) {
                    $("tbody").html("");
                    load_calendar(
                      moment($("#date-jump-text").val(), "YYYY-MM-DD"),
                      sessionStorage.getItem("uid")
                    );
          ...
          database
            ...
            .then(function(snap) {
              ...
              snap.forEach(function(value) {
                ...
                var event_datetime =
                  value.val().start_date + " " + value.val().start_time;
                var reminder = "";
                if (
                  moment()
                    .add(reminder_warning, "minutes")
                    .format("YYYY-MM-DD HH:mm") >= event_datetime &&
                  moment().format("YYYY-MM-DD HH:mm") < event_datetime
                ) {
                  reminder =
                    "<b>" +
                    value.val().event_title +
                    "</b> is coming up at <b>" +
                    moment(event_datetime, "YYYY-MM-DD HH:mm").format(
                      "MMM D, YYYY h:mm A"
                    ) +
                    "</b>.";
